...
### Проверка текстовых промптов (OpenAI Moderation)
1. Каждый промпт пользователя отправляется в OpenAI Moderation API (`omni-moderation-latest`) до генерации изображения.
2. Если OpenAI помечает текст как `flagged`, мы фиксируем список нарушенных категорий и блокируем промпт.
...
Ответ модерации:
  - Сервис: {{Google SafeSearch|OpenAI Moderation}}
  - Категория: {{category}}
...
### Быстрая реакция
- Любое срабатывание SafeSearch или OpenAI Moderation с высоким уровнем — повод для немедленного отключения пользователя от сервиса.
