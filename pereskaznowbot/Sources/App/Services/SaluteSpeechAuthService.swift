...
actor SaluteSpeechAuthService {
    ...
    func currentToken(forceRefresh: Bool = false, logger: Logger) async throws -> String {
        guard authKey.isEmpty == false else {
            ...
            throw Abort(.internalServerError, reason: "SALUTESPEECH_AUTH_KEY не задан в config/.env")
        }
    ...
    private func fetchToken(logger: Logger) async throws -> CachedToken {
        ...
        guard var responseBody = response.body,
            ...
            throw Abort(.internalServerError, reason: "Не удалось получить токен SaluteSpeech (пустой ответ)")
        }
        ...
        guard response.status == .ok else {
            ...
            throw Abort(.internalServerError, reason: "SaluteSpeech ошибка при выдаче токена: \(reason)")
        }
        ...
        logger.info("SaluteSpeechAuthService: получен токен, истекает \(expiryDate)")
