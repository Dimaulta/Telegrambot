...
private func loadDotEnvIfAvailable(logger: Logger) {
    ...
    guard let content = try? String(contentsOfFile: envPath) else {
        logger.debug("config/.env not found for SoranowBot")
        return
    }
    ...
    logger.info("Loaded config/.env with \(vars.count) keys for SoranowBot")
}
...
public func configure(_ app: Application) async throws {
    ...
    if let proxyString = Environment.get("SORANOWBOT_HTTP_PROXY"),
       let proxyURL = URL(string: proxyString),
       let host = proxyURL.host,
       let port = proxyURL.port {
        var configuration = app.http.client.configuration
        if let user = Environment.get("SORANOWBOT_PROXY_USER"),
           let password = Environment.get("SORANOWBOT_PROXY_PASS") {
