...
⚠️ Важно: ngrok для пользователей из России запускается только в терминале с VPN. URL будет стабильным неделями
...
> Подробный план настройки GSForTextBot с ключами и сертификатами см. в `gsfortextbot/docs/SETUP_PLAN.md`.
...
7. Проверочные команды — см. `docs/VERIFY.md`. Запускай их в отдельной вкладке (Cmd + T) и закрывай после завершения тестов
