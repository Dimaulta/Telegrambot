...
enum MonetizationService {
    ...
    static func ensureDatabase(app: Application) {
        ...
        do {
            ...
            app.logger.error("Failed to create directory for monetization DB (gsfortextbot): \(error.localizedDescription)")
        }

        var db: OpaquePointer?
        if sqlite3_open(path, &db) != SQLITE_OK {
            if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                app.logger.error("Failed to open monetization DB at \(path) (gsfortextbot): \(errorMessage)")
            } else {
                app.logger.error("Failed to open monetization DB at \(path) (gsfortextbot)")
            }
            if db != nil {
                sqlite3_close(db)
            }
        ...
        for sql in [createUserSessionsSQL, createSponsorCampaignsSQL, createBotSettingsSQL] {
            if sqlite3_exec(db, sql, nil, nil, nil) != SQLITE_OK {
                if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                    app.logger.error("Failed to run schema SQL (gsfortextbot): \(errorMessage)")
                } else {
                    app.logger.error("Failed to run schema SQL (gsfortextbot, unknown error)")
                }
            }
        }

        app.logger.info("Monetization DB ensured at path (gsfortextbot): \(path)")
    }
