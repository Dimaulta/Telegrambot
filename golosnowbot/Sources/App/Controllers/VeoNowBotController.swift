...
struct GolosNowBotController: Sendable {
    private let ttsApiClient: TtsApiClient

    init(ttsApiClient: TtsApiClient = TtsApiClient()) {
        self.ttsApiClient = ttsApiClient
    }
    ...
    func handleWebhook(_ req: Request) async throws -> Response {
        ...
        let botToken = Environment.get("GOLOSNOWBOT_TOKEN") ?? ""
        guard botToken.isEmpty == false else {
            req.logger.error("GOLOSNOWBOT_TOKEN is not configured")
