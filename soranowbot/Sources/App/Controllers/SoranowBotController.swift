...
final class SoranowBotController {
    ...
    init() {
        self.botToken = Environment.get("SORANOWBOT_TOKEN") ?? ""
    ...
    func handleWebhook(_ req: Request) async throws -> Response {
        guard botToken.isEmpty == false else {
            req.logger.error("SORANOWBOT_TOKEN is not configured")
            return Response(status: .ok)
        }

        let update: SoranowBotUpdate
        do {
            update = try req.content.decode(SoranowBotUpdate.self)
        } catch {
            req.logger.error("SoranowBotController: failed to decode update: \(error)")
            return Response(status: .ok)
        }
        ...
        do {
            ...
            req.logger.error("SoranowBotController: API error \(abortError.reason)")
            try? await sendMessage(
                on: req,
                chatId: chatId,
            ...
            req.logger.error("SoranowBotController: unexpected error \(error)")
            try? await sendMessage(
                on: req,
                chatId: chatId,
    ...
    private func sendMessage(on req: Request, chatId: Int64, text: String) async throws {
        ...
        if response.status != .ok {
            req.logger.warning("SoranowBotController: sendMessage returned status \(response.status.code)")
        }
    }
    ...
    private func sendVideo(on req: Request, chatId: Int64, videoURL: String, caption: String?) async throws {
        ...
        if response.status != .ok {
            req.logger.warning("SoranowBotController: sendVideo returned status \(response.status.code)")
            try await sendMessage(
                on: req,
                chatId: chatId,
