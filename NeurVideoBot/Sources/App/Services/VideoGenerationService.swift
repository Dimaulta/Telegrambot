...
final class VideoGenerationService {
    ...
    init() {
        self.apiKey = Environment.get("NEURVIDEOBOT_API_KEY")
    }
    ...
    func generateVideo(prompt: String, on req: Request) async throws -> VideoGenerationResult {
        ...
        guard let endpointString = Environment.get("NEURVIDEOBOT_API_URL"),
              endpointString.isEmpty == false else {
            throw Abort(.internalServerError, reason: "NEURVIDEOBOT_API_URL is not configured")
        }
