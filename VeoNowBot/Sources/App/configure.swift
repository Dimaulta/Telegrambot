...
private func loadEnvironmentVariables(logger: Logger) {
    let envPath = "VeoNowBot/config/.env"
    ...
    logger.info("Loaded \(vars.count) environment variables for VeoNowBot")
}
...
public func configure(_ app: Application) async throws {
    ...
    let configuredPort = Environment.get("VEONOWBOT_PORT").flatMap(Int.init)
        ?? getPortFromConfig(serviceName: "veonowbot")
        ?? 8087
