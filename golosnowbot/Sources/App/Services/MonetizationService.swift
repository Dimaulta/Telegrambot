...
enum MonetizationService {
    ...
    static func ensureDatabase(app: Application) {
        ...
        do {
            ...
            app.logger.error("Failed to create directory for monetization DB (golosnowbot): \(error.localizedDescription)")
        }
        ...
        if result != SQLITE_OK {
            if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                app.logger.error("Failed to open monetization DB at \(path) (golosnowbot): \(errorMessage)")
            } else {
                app.logger.error("Failed to open monetization DB at \(path) (golosnowbot)")
            }
            if db != nil {
                sqlite3_close(db)
            }
        ...
        for sql in [createUserSessionsSQL, createSponsorCampaignsSQL, createBotSettingsSQL] {
            ...
            if execResult != SQLITE_OK {
                if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                    app.logger.error("Failed to run schema SQL (golosnowbot): \(errorMessage)")
                } else {
                    app.logger.error("Failed to run schema SQL (golosnowbot, unknown error)")
                }
            }
        }
        ...
        app.logger.info("Monetization DB ensured at path (golosnowbot): \(path)")
    }
