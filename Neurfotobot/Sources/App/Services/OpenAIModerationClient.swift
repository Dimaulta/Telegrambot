    ...
    func analyze(text: String) async throws -> AnalysisResult {
        ...
        for attempt in 1...maxRetries {
            do {
                ...
                // Обрабатываем 429 (Too Many Requests) с retry
                if response.status.code == 429 {
                    if attempt < maxRetries {
                        let delay = min(Double(attempt) * 2.0, 10.0) // Exponential backoff: 2s, 4s, 8s (max 10s)
        ...
        throw lastError ?? Abort(.badRequest, reason: "OpenAI moderation failed after \(maxRetries) attempts")
    }
}
