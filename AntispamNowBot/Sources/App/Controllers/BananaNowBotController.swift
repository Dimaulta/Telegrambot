...
final class AntispamNowBotController {
    private let antiSpamService: AntiSpamService

    init(antiSpamService: AntiSpamService = AntiSpamService()) {
        self.antiSpamService = antiSpamService
    }

    func handleWebhook(_ req: Request) async throws -> Response {
        guard let token = Environment.get("ANTISPAMNOWBOT_TOKEN"), token.isEmpty == false else {
            req.logger.error("ANTISPAMNOWBOT_TOKEN отсутствует — проверь config/.env")
            return Response(status: .internalServerError)
        }

        guard let update = try? req.content.decode(AntispamNowBotUpdate.self) else {
            req.logger.warning("Не удалось декодировать AntispamNowBotUpdate")
        ...
        if text == "/start" {
            try await sendTelegramMessage(
                ...
                chatId: chatId,
                text: """
                Привет! 🛡️ Я AntispamNowBot — антиспам бот.
                Функции: выключатель на ночь, капча для вступления, запрещатель сообщений от каналов.
                """,
                client: req.client
            )
            return Response(status: .ok)
