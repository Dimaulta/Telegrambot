...
struct TelegramChannelService {
    ...
    func getChannelPosts(chatId: Int64, limit: Int = 10) async throws -> [ChannelPost] {
        ...
        // Поэтому мы полагаемся на два сценария:
        // 1. Пользователь пересылает (Forward) существующие посты в чат с ботом.
        // 2. (Опционально) бот хранит посты при публикации, если его сделали админом.
        ...
        // Пока возвращаем пустой массив — основной источник данных у нас пересланные посты от автора.
