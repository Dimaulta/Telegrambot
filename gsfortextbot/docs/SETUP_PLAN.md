...
- **SaluteSpeech (Сбер)** — регистрируемся в [Sber Developers Studio](https://developers.sber.ru/studio), создаём проект *SaluteSpeech API*.
  - 🔗 [Главная страница Studio](https://developers.sber.ru/studio) — вход/регистрация
  - 🔗 [SmartSpeech (SaluteSpeech)](https://developers.sber.ru/portal/solutions/smartspeech) — информация о сервисе
  - 🔗 [Документация SmartSpeech](https://developers.sber.ru/docs/ru/smartspeech/overview) — официальная документация API
...
1. **Вход/регистрация в Studio:**
   - Переходим на [Sber Developers Studio](https://developers.sber.ru/studio)
   - Если нет аккаунта — регистрируемся (требуется подтверждение через СМС/email)
   - Входим в личный кабинет

2. **Создание проекта SaluteSpeech:**
   - В Studio создаём новый проект или выбираем существующий
   - Выбираем тип проекта: **SaluteSpeech API** (SmartSpeech)
   - Если проекта нет — создаём через кнопку "Создать проект" → выбираем "SmartSpeech"

3. **Получение ключей:**
   - Открываем проект в Studio → **Настройки API** (или **API Keys**)
   - Нажимаем «Получить ключ» / «Создать ключ»
   - Сохраняем `Authorization key` (Base64-строка) — кладём в `.env` как `SALUTESPEECH_AUTH_KEY`
   - Сохраняем `Client ID` (не нужен напрямую, он уже зашит в `Authorization key`)

4. **Scope:**
   - Для физ. лица — `SALUTE_SPEECH_PERS`
   - Для юрлица — другой scope (см. [документацию](https://developers.sber.ru/docs/ru/smartspeech/overview))
