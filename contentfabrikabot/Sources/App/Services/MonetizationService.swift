...
/// Сервис монетизации для contentfabrikabot.
/// Отвечает за:
...
enum MonetizationService {
    ...
    static func ensureDatabase(app: Application) {
        ...
        do {
            ...
            app.logger.error("Failed to create directory for monetization DB (contentfabrikabot): \(error.localizedDescription)")
        }
        ...
        if result != SQLITE_OK {
            if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                app.logger.error("Failed to open monetization DB at \(path) (contentfabrikabot): \(errorMessage)")
            } else {
                app.logger.error("Failed to open monetization DB at \(path) (contentfabrikabot)")
            }
            if db != nil {
                sqlite3_close(db)
            }
        ...
        for sql in [createUserSessionsSQL, createSponsorCampaignsSQL, createBotSettingsSQL] {
            ...
            if execResult != SQLITE_OK {
                if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                    app.logger.error("Failed to run schema SQL (contentfabrikabot): \(errorMessage)")
                } else {
                    app.logger.error("Failed to run schema SQL (contentfabrikabot, unknown error)")
                }
            }
        }
        ...
        app.logger.info("Monetization DB ensured at path (contentfabrikabot): \(path)")
    }
