...
enum MonetizationService {
    ...
    static func ensureDatabase(app: Application) {
        ...
        do {
            ...
            app.logger.error("Failed to create directory for monetization DB (pereskaznowbot): \(error.localizedDescription)")
        }
        ...
        if result != SQLITE_OK {
            if let errorMessage = sqlite3_errmsg(db).flatMap({ String(cString: $0) }) {
                ...
                app.logger.error("Failed to open monetization DB at \(path) (pereskaznowbot)")
            }
            if db != nil {
                sqlite3_close(db)
            }
